 ...
 * Populates MongoDB Atlas with sample data
 *
...
async function seed() {
    ...
    // Clear existing data
    const prodDel = await Produto.deleteMany({});
    const matDel = await Material.deleteMany({});
    const venDel = await Venda.deleteMany({});
    console.log(`  ✓ Removidos: ${prodDel.deletedCount} produtos, ${matDel.deletedCount} materiais, ${venDel.deletedCount} vendas\n`);

    // Insert produtos
    const produtosCriados = await Produto.insertMany(produtosData);
    console.log(`  ✓ ${produtosCriados.length} produtos inseridos`);

    // Insert materiais
    const materiaisCriados = await Material.insertMany(materiaisData);
    ...
    const vendasData = [{
        ...
        formaPagamento: 'Pix',
        status: 'pendente',
        dataVenda: new Date(2026, 4, 12)
    }, ];

    const vendasCriadas = await Venda.insertMany(vendasData);
